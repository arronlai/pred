...
const handleStartPrediction = async () => {
  ...
  if (!numbers.value[0] || !numbers.value[1] || !numbers.value[2]) {
    uni.showToast({
      title: '请在心里默念您的问题，输入3个数字（1-100之间）',
      icon: 'none',
    });
    return;
  }
  ...
  try {
    ...
    if (result.result) {
      if (result.result.code === 0) {
        ...
        throw new Error(result.result.message || '生成解读失败');
      }
    } else {
    ...
    uni.showToast({
      title: error.message || '生成解读失败，请重试',
      icon: 'none',
    });
